...
- Ja nav iespējams noteikt informāciju vai tā nav zināma, ievieto `-`.
- Nepieņem minējumus – tikai redzama informācija.
- Ja prece ir pirmās nepieciešamības pārtikas produkts, tad Groza prece ir `1`. Visas pārējās preces ir `0`.
...
- Cenas pieraksti, izmantojot komatu kā decimālatdalītāju (piemēram, `2,99`).
- Norādot cenas, nelieto valūtas simbolus (piemēram, € vai EUR).
- Cena ir pārdošanas cena pirms akcijas atlaides vai lojalitātes kartes atlaides piemērošanas.
- Ja produkta vienība ir bez skaitļa, tad ievieto `1`.
- Ja mērvienība ir gab, tad raskti ar lielo burtu un punktu beigās (piemēram, `Gab.`).
- Ja produkta mērvienība ir grami, bet vienības cena ir kilogramos, tad salāgo Produkta vienību, lai atbilst mērvienībai (piemēram, `500 Grami` = `0,5 Kg`).   
- Ja svītrkods sastāv no divu skaitļu virknes, tad norādi garāko vai kurš sākas ar 475.
- Svītrkods nekad nesatur punktus un parasti ir izvietots zem vai pa labi no stabiņveida līnijām.
- Piezīmēs norādi būtisku informāciju, kas varētu būt noderīga, piemēram, ražotāja nosaukumu.
- Grozs ir jānorāda kā `0`.
"""
...
def process_image(uploaded_file, use_metadata, custom_prompt, employee, merchant, city, store_address, date_value, time_value, client):
    try:
        ...
        # Veicam pieprasījumu uz Groq klientu ar attēlu un uzvedni
...
if process:
    if not employee.strip():
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    process_image, f, use_metadata, custom_prompt, employee,
